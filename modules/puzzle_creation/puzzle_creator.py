  ...
  for i in range(0, puzzle_mask.shape[0], piece_size):
    for j in range(0, puzzle_mask.shape[1], piece_size):
      ...
      if i != 0:
        ...
        if (j//piece_size)%2:
          cv2.ellipse(puzzle_mask, (j + l_j, i), (l_j, 2), 0, 180, 280, 255, thickness=2, lineType = cv2.LINE_8)
          cv2.ellipse(puzzle_mask, (j + r_j, i), (piece_size - r_j, 2), 0, 260, 360, 255, thickness=2, lineType = cv2.LINE_8)
          offset = -2
        else:
          cv2.ellipse(puzzle_mask, (j + l_j, i), (l_j, 2), 0, 80, 180, 255, thickness=2, lineType = cv2.LINE_8)
          cv2.ellipse(puzzle_mask, (j + r_j, i), (piece_size - r_j, 2), 0, 0, 100, 255, thickness=2, lineType = cv2.LINE_8)
          offset = 2

        # Select the piece hole direction
        if np.random.randint(1,3) == 1:
          # Draw upwards  
          cv2.ellipse(puzzle_mask, (j + mid_j, i + offset - 2*half_joint_size), ((r_j - l_j)//2, (5*half_joint_size)//2), 90, 40, 320, 255, thickness=2, lineType = cv2.LINE_8)

        else:
          # Draw downwards
          cv2.ellipse(puzzle_mask, (j + mid_j, i + offset + 2*half_joint_size), ((r_j - l_j)//2, (5*half_joint_size)//2), 270, 40, 320, 255, thickness=2, lineType = cv2.LINE_8)
      ...
      if j != 0:
        ...
        # Draw borders of piece
        if (i//piece_size)%2:
          cv2.ellipse(puzzle_mask, (j, i + l_i), (2, l_i), 0, 270, 370, 255, thickness=2, lineType = cv2.LINE_8)
          cv2.ellipse(puzzle_mask, (j, i + r_i), (2, l_i), 0, -10, 90, 255, thickness=2, lineType = cv2.LINE_8)
          offset = 2
        else:
          cv2.ellipse(puzzle_mask, (j, i + l_i), (2, l_i), 0, 170, 270, 255, thickness=2, lineType = cv2.LINE_8)
          cv2.ellipse(puzzle_mask, (j, i + r_i), (2, l_i), 0, 90, 190, 255, thickness=2, lineType = cv2.LINE_8)
          offset = -2
          
        # Select the piece hole direction
        if np.random.randint(1,3) == 1:
          # Draw to the left
          cv2.ellipse(puzzle_mask, (j + offset - 2*half_joint_size, i + mid_i), ((5*half_joint_size)//2, (r_i - l_i)//2), 0, 40, 320, 255, thickness=2, lineType = cv2.LINE_8)

        else:
          # Draw to the right
          cv2.ellipse(puzzle_mask, (j + offset + 2*half_joint_size, i + mid_i), ((5*half_joint_size)//2, (r_i - l_i)//2), 180, 40, 320, 255, thickness=2, lineType = cv2.LINE_8)
